<template>
  <STitleContentPanel>
    ...
    <div class="chart-container">
      <SChart id="aChart" :chartOptions="option" :update-options="updateOption" autoresize />
    </div>
  </STitleContentPanel>
</template>

<script setup lang="ts">
//@名称:房屋建筑和市政设施(FangWuShiZhengChart.vue)
//@作者:lvp
//@日期:
//@修改：Jin 2022/12/31
//@说明:
import { ref, onMounted } from "vue";
import { EChartsOption, SeriesOption, SetOptionOpts } from "echarts";
//设置当前组件的id，用于标识当前组件
const componentName = "CChart";
/**
 * 定义props
 */
...
let updateOption = ref<SetOptionOpts>({
  notMerge: true,
});
...
function getOption(): EChartsOption {
  ...
  let seriesOption: SeriesOption[] = [
      ...
      renderItem: function (params: any, api: any) {
        return {
          ...
          shape: {
            cx: api.getWidth() / 2,
            cy: api.getHeight() / 2,
            r: Math.min(api.getWidth(), api.getHeight()) / 2.2,
    ...
    },
    {
      ...
      renderItem: function (params: any, api: any) {
        ...
        let r = Math.min(api.getWidth(), api.getHeight()) / 2.2;
    ...
    },
    {
      ...
      renderItem: function (params: any, api: any) {
        let x0 = api.getWidth() / 2;
        let y0 = api.getHeight() / 2;
        let r = Math.min(api.getWidth(), api.getHeight()) / 2.4;
  ...
  return {
    ...
    legend: {
      ...
      orient: "horizontal",
      // x: 'left',
      data: [optionData[0].name, optionData[1].name],
...
function draw() {
  ...
  let rotateTimeout = requestAnimationFrame(function () {
    draw();
  });
